...
#'
#' Produces a table of fit statistics for multiple lm models: AIC, AICc, BIC,
#' p-value, R-squared, and adjusted R-squared.
#'
#' Smaller values for AIC, AICc, and BIC indicate a better balance of
#' goodness-of-fit of the model and the complexity of the model. The goal is to
#' find a model that adequately explains the data without having too many terms.
#'
#' BIC tends to choose models with fewer parameters relative to AIC.
#'
#' In the table, \code{Shapiro.W} and \code{Shapiro.p} are the W statistic and
#' p-value for the Shapiro-Wilks test on the residuals of the model.
#'
#' For comparisons with AIC, etc., to be valid, both models must have the same
#' data, without transformations, use the same dependent variable, and be fit
#' with the same method. They do not need to be nested.
#'
...
#' @return A list of two objects: The series of model calls, and a data frame of
...
#'              data = BrendonSmall)
#' bootcamp::compareLM(model.1, model.2, model.3, model.4)
#' 
...
compareLM <- function (fits, ...) {
    ...
    for(i in 1:n) {
      ...
      Z[i,]=c(signif(fits[[i]]$rank, digits=4),
              ...
              signif(as.numeric(stats::shapiro.test(fits[[i]]$residuals)$statistic),
                     digits =4),
              signif(stats::shapiro.test(fits[[i]]$residuals)$p.value,
                     digits=4)
      )
    }     
